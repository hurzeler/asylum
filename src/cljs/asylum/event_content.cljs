...
(def mou-naru
  {:title "Australian Government - MOU with Nauru"
   ...
   :constraints {:morrison [0.05 0.95]
                 :turn [1 100]}
    :options
   {:good {:effect (ee/dsr-factor :morrison -0.02) :title "Decline" :description ""}
    :bad {:effect (ee/dsr-factor :morrison 0.01) :title "Sign MOU" :description "Sing the MOU"}}})
...
(def excision-policy 
  {:title "Extend excision policy to the Australian mainland"
   ...
   {:good {:effect (ee/dsr-factor :morrison -0.03) :title "Reject" :description ""}
    :bad {:effect (ee/dsr-factor :morrison 0.05) :title "Enact" :description "Pass excission policy legislation"}}})
...
(def resettlemet-deal-new-zealand
  {:title "Resettlement deal with New Zealand"
   :media {:name "nz-refugees.jpg" :type :image}
   :content "<p>The Australian Government considers a new resettlement deal with New Zealand, whereby 150 places from New Zealand's overall resettlement quota of 750 will be allocated to refugees who sought asylum in Australia. The refugees may be resettled directly from Australia or from offshore processing centres in Nauru and Manus Island.</p>"
...
(def unhcr-nauru-report
  {:title "UNHCR Nauru condition report"
   ...
   :content "<p>UNHCR releases a report on conditions in the offshore processing facility in Nauru. It concludes that the transfer of asylum-seekers to what are currently harsh and unsatisfactory temporary facilities, within a closed detention setting, and in the absence of a fully functional legal framework and adequately capacitated system to assess refugee claims, do not currently meet the required protection standards. </p>"
...
(def no-advantage-test
  {:title "No Advantage Test"
   ...
   :constraints {:morrison [0.2 0.95]
                 :turn [2 100]}
    :options
   {:good {:effect (ee/dsr-factor :morrison -0.03) :title "Decline" :description ""}
    :bad {:effect (ee/dsr-factor :morrison 0.07) :title "Introduce Test" :description "You accept that a no-advantage test is necessary"}}})
...
(def nauru-transfer
  {:title "Offshore processing in Nauru"
   ...
   :constraints {:morrison [0.05 0.95]
                 :turn [1 100]}
    :options
   {:good {:effect (ee/dsr-factor :morrison -0.02) :title "Decline" :description "You are not in favour of offshore processing"}
...
(def expert-panel-angus-houston
  {:title ""
   ...
   {:good {:effect (ee/dsr-factor :morrison 0.05) :title "Implement" :description "You appoint an expert panel"}
    :bad {:effect (ee/dsr-factor :morrison -0.05) :title "Decline" :description "You reconsider and will not appoint an expert panel"}}})
